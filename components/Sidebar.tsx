...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <div className={`
        fixed inset-y-0 left-0 z-50 w-80 bg-white border-r border-slate-200 shadow-xl transform transition-transform duration-300 ease-in-out
        md:static md:translate-x-0 md:shadow-none h-full flex flex-col
        ...
        <div className="p-4 border-b border-slate-100 bg-white z-20 flex-shrink-0 space-y-3">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wide">
                 Total: <span className="text-blue-600 font-bold">{questions.length}</span> | Visible: <span className="text-blue-600 font-bold">{visibleCount}</span>
              </p>
            </div>
          ...
          <div className="flex bg-slate-100 p-1 rounded-lg">
             <button
               ...
               className={`flex-1 py-1 text-xs font-semibold rounded transition-all ${
                 selectedLevel === 'AS' ? 'bg-white text-blue-700 shadow-sm font-bold' : 'text-slate-500 hover:text-slate-700'
               }`}
             >
               AS Level
             </button>
             <button
               ...
               className={`flex-1 py-1 text-xs font-semibold rounded transition-all ${
                 selectedLevel === 'A Level' ? 'bg-white text-purple-700 shadow-sm font-bold' : 'text-slate-500 hover:text-slate-700'
               }`}
             >
               A Level
             </button>
          ...
          <div className="grid grid-cols-2 gap-2">
              <select 
                ...
                className="bg-slate-50 border border-slate-200 text-slate-700 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 outline-none cursor-pointer"
              >
          ...
          {/* Sort & Filter Mode Row */}
          <div className="flex justify-between items-center border-t border-slate-100 pt-2 gap-2">
             ...
             <div className="flex gap-3 text-xs font-medium">
                <button 
                  ...
                  className={`hover:text-blue-600 ${filterMode === 'all' ? 'text-blue-600 underline decoration-2 underline-offset-4' : 'text-slate-400'}`}
                >
                  All
                </button>
                <button 
                  onClick={() => setFilterMode('saved')}
                  className={`hover:text-emerald-600 ${filterMode === 'saved' ? 'text-emerald-600 underline decoration-2 underline-offset-4' : 'text-slate-400'}`}
                >
                  Work
                </button>
             </div>
          ...
          {sortOption === 'syllabus' && syllabusGroups && (
             <>
               {orderedTopics.map((topic) => {
                 ...
                 const topicQCount = Object.values(chapters).reduce((acc, curr) => acc + curr.length, 0);

                 return (
                   <div key={topic} className="mb-2">
                     <button 
                        onClick={() => toggleTopic(topic)}
                        className="w-full flex justify-between items-center px-3 py-2 bg-white hover:bg-slate-50 border border-slate-100 rounded-lg transition-colors group sticky top-0 z-10"
                     >
                        <div className="flex items-center gap-2 overflow-hidden">
                            <span className={`text-xs transition-transform duration-200 ${isExpanded ? 'rotate-90 text-blue-500' : 'text-slate-400'}`}>▶</span>
                            <h2 className={`text-[10px] font-bold uppercase tracking-widest truncate ${isExpanded ? 'text-blue-700' : 'text-slate-600'}`}>
                              {topic.replace(/^\d+\.\s*/, '')}
                            </h2>
                        </div>
                        <span className="text-[10px] font-medium text-slate-400 bg-slate-100 px-1.5 rounded-full">{topicQCount}</span>
                     </button>
                     
                     {isExpanded && (
                       <div className="mt-2 pl-2 border-l-2 border-slate-100 ml-3 space-y-4 animate-slide-down">
                         {sortedChapters.map((chapter) => {
                           ...
                           return (
                             <div key={chapter}>
                               <h3 className="text-[11px] font-bold text-slate-500 mb-2 pl-2 select-none" title={chapter}>
                                 {chapter.split(' ').slice(0, 1)} <span className="font-normal text-slate-400">{chapter.split(' ').slice(1).join(' ')}</span>
                               </h3>
...
}> = ({ q, isSelected, questionStates, onSelect, onEdit, onDelete }) => {
